...
function vRPN.nuIdentidade()
	...
	if nplayer then
		...
		if user_id then
			...
			TriggerEvent("unity_logs:send", "Inventory", "Identidade Verificada", message, 3066993, source)
		end
				...
				if sobrenome ~= "" then
					...
					if idade ~= "" then
						...
						if idade >= "18" and idade <= "90" then
                            ...
                            TriggerEvent("unity_logs:send", "Inventory", "Identidade Atualizada", message, 3066993, source)
