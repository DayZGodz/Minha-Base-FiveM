			...
			if parseInt(amount) > 0 then
				if vRP.getInventoryWeight(nuser_id) + vRP.getItemWeight(itemName) * amount <= vRP.getInventoryMaxWeight(nuser_id) then
					if vRP.tryGetInventoryItem(user_id,itemName,amount) then
                        ...
                        local message = "**Quem Enviou:** " .. identity.name .. " " .. identity.firstname .. " [" .. user_id .. "]\n**Quem Recebeu:** " .. identitynu.name .. " " .. identitynu.firstname .. " [" .. nuser_id .. "]\n**Item:** " .. vRP.itemNameList(itemName) .. "\n**Quantidade:** " .. vRP.format(parseInt(amount)) .. "\n**Local:** " .. string.format("%.2f, %.2f, %.2f", x, y, z)
                        TriggerEvent("unity_logs:send", "Inventory", "Item Enviado", message, 3066993, source)
	...
	if itemName then
		if itemName == "passaporte" then
			...
			if parseInt(amount) > 0 and vRP.tryGetInventoryItem(user_id,itemName,amount) then
                ...
                local message = "**Quem Dropou:** " .. identity.name .. " " .. identity.firstname .. " [" .. user_id .. "]\n**Item:** " .. vRP.itemNameList(itemName) .. "\n**Quantidade:** " .. vRP.format(parseInt(amount)) .. "\n**Local:** " .. string.format("%.2f, %.2f, %.2f", x, y, z)
                TriggerEvent("unity_logs:send", "Inventory", "Item Dropado", message, 15158332, source)
			...
			elseif itemName == "identidade" then
				...
				if nplayer then
					...
					if identity then
                        local nuser_id = vRP.getUserId(nplayer)
                        local identitynu = vRP.getUserIdentity(nuser_id)
                        local message = "**Quem Mostrou:** " .. identity.name .. " " .. identity.firstname .. " [" .. user_id .. "]\n**Quem Viu:** " .. identitynu.name .. " " .. identitynu.firstname .. " [" .. nuser_id .. "]"
                        TriggerEvent("unity_logs:send", "Inventory", "Identidade Apresentada", message, 3447003, source)
