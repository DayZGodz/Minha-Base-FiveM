...
local godz_wall = Tunnel.getInterface("godz_modules_wall")
...
RegisterCommand("wall", function(source, args, rawCommand)
	if godz_wall.getPermissao() then
		wallEnabled = not wallEnabled
	end
end, false)

-- Evento para atualizar a tabela de players enviada pelo servidor
RegisterNetEvent("godz_modules:updatePlayers")
AddEventHandler("godz_modules:updatePlayers", function(ids)
    players = ids
end)
