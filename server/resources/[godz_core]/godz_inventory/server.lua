...
vRPN = {}
Tunnel.bindInterface("godz_inventory",vRPN)
Proxy.addInterface("godz_inventory",vRPN)

local idgens = Tools.newIDGenerator()
local groups = cfg.groups

vCLIENT = Tunnel.getInterface("godz_inventory")
vGARAGE = Tunnel.getInterface("godz_garages")
vDIAGNOSTIC = Tunnel.getInterface("godz_modules_ferimentos")
--------------------------------------------------------------
-- WEBHOOK
--------------------------------------------------------------
	...
	if user_id and ramount ~= nil and parseInt(ramount) >= 0 and not actived[user_id] and actived[user_id] == nil then
		if type == "usar" then
			if itemName == "valemansao" then
                TriggerClientEvent('godz_inventory:Close',source)
