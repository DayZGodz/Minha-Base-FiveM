...
policia_tunnel = {}
Tunnel.bindInterface("godz_modules_policia",policia_tunnel)
...
-- RG
-----------------------------------------------------------------------------------------------------------------------------------------
local function verificar_ficha(source)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"polpar.permissao") or vRP.hasPermission(user_id,"cpolicia.permissao") or vRP.hasPermission(user_id,"juiz.permissao") or vRP.hasPermission(user_id,"policia.permissao") then
		local nplayer = vRPclient.getNearestPlayer(source,2)
		if nplayer then
			vRPclient._playAnim(source,true,{{"amb@world_human_clipboard@male@idle_a","idle_a"}},true)
			local nuser_id = vRP.getUserId(nplayer)
			if nuser_id then
...
RegisterCommand('rg',function(source,args,rawCommand)
	...
	if args[1] then
		...
		verificar_ficha(source)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- ALGEMAR
-----------------------------------------------------------------------------------------------------------------------------------------
local function algemar_player(source)
	local user_id = vRP.getUserId(source)
	local nplayer = vRPclient.getNearestPlayer(source,2)
	if nplayer then
...
AddEventHandler("vrp_policia:algemar",function()
	algemar_player(source)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- CARREGAR
-----------------------------------------------------------------------------------------------------------------------------------------
local function carregar_player(source)
	local user_id = vRP.getUserId(source)
	local nplayer = vRPclient.getNearestPlayer(source,2)
	if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"polpar.permissao") or vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"cpolicia.permissao") then	
		if nplayer then
			if not vRPclient.isHandcuffed(source) then
				TriggerClientEvent('carregar',nplayer,source)
			end
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- APREENDER ITENS (REFACTORED)
-----------------------------------------------------------------------------------------------------------------------------------------
local function apreender_itens(source)
    local user_id = vRP.getUserId(source)
    if vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"cpolicia.permissao") or vRP.hasPermission(user_id,"admin.permissao") then
        local nplayer = vRPclient.getNearestPlayer(source,2)
        if nplayer then
            -- Animation
            vRPclient._playAnim(source,true,{{"oddjobs@shop_robbery@rob_till","loop"}},true)
            TriggerClientEvent("Notify",source,"importante","Apreendendo itens...")
            ...
            SetTimeout(3000, function()
                ...
                if nuser_id then
                    ...
                    if #itens_apreendidos > 0 then
                        local apreendidos = table.concat(itens_apreendidos, "\n")
                        SendWebhookMessage(webhookpoliciaapreendidos,"```prolog\n[ID]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[APREENDEU DE]:  "..nuser_id.." "..nidentity.name.." "..nidentity.firstname.."\n" .. apreendidos ..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
                    end
                    TriggerClientEvent("Notify",nplayer,"importante","Todos os seus pertences ilegais foram apreendidos.")
                    TriggerClientEvent("Notify",source,"sucesso","Apreensão concluída.")
                end
            end)
        else
            TriggerClientEvent("Notify",source,"negado","Nenhum cidadão próximo.")
...
-- MENU POLICIA
-----------------------------------------------------------------------------------------------------------------------------------------
local function abrirMenuPolicia(source)
    local user_id = vRP.getUserId(source)
    if vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"cpolicia.permissao") or vRP.hasPermission(user_id,"admin.permissao") then
        local menudata = {name="Polícia",css={top="75px",header_color="rgba(0,125,255,0.75)"}}
        
        menudata["Verificar Ficha"] = {function(player,choice)
            verificar_ficha(player)
        end, "Verificar identidade e multas."}
        
        menudata["Apreender Itens"] = {function(player,choice)
            apreender_itens(player)
        end, "Apreender armas e itens ilegais."}
        
        menudata["Escoltar"] = {function(player,choice)
            carregar_player(player)
        end, "Arrastar/Soltar cidadão."}
        
        menudata["Colocar no Veículo"] = {function(player,choice)
            cv_player(player)
        end, "Colocar cidadão no veículo próximo."}
        
        menudata["Retirar do Veículo"] = {function(player,choice)
            rv_player(player)
        end, "Retirar cidadão do veículo."}

        menudata["Algemar"] = {function(player,choice)
            algemar_player(player)
        end, "Algemar/Desalgemar cidadão."}

        vRP.openMenu(source,menudata)
    end
end

RegisterServerEvent("vrp_policia:openMenu")
AddEventHandler("vrp_policia:openMenu", function()
    local source = source
    abrirMenuPolicia(source)
end)

RegisterServerEvent("vrp_policia:verificarFicha")
AddEventHandler("vrp_policia:verificarFicha", function()
    local source = source
    verificar_ficha(source)
end)

RegisterServerEvent("vrp_policia:apreenderItens")
AddEventHandler("vrp_policia:apreenderItens", function()
    local source = source
    apreender_itens(source)
end)

RegisterCommand('pmenu', function(source,args,rawCommand)
    abrirMenuPolicia(source)
end)
