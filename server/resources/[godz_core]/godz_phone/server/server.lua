...
src = {}
Tunnel.bindInterface("godz_phone",src)
vRPclient = Tunnel.getInterface("vRP", "godz_phone")
...
function getNumberPhone(identifier)
	local result = MySQL.Sync.fetchAll("SELECT godz_user_identities.phone FROM godz_user_identities WHERE godz_user_identities.user_id = @identifier",{ ['@identifier'] = identifier })
...
function getIdentifierByPhoneNumber(phone_number) 
	local result = MySQL.Sync.fetchAll("SELECT godz_user_identities.user_id FROM godz_user_identities WHERE godz_user_identities.phone = @phone_number",{ ['@phone_number'] = phone_number })
	if result[1] ~= nil then
...
function getOrGeneratePhoneNumber(sourcePlayer,identifier,cb)
	...
	if myPhoneNumber == '0' or myPhoneNumber == nil then
		...
		until id == nil
		MySQL.Async.insert("UPDATE godz_user_identities SET phone = @myPhoneNumber WHERE user_id = @identifier",{ ['@myPhoneNumber'] = myPhoneNumber, ['@identifier'] = identifier },function()
			cb(myPhoneNumber)
		end)
	else
		cb(myPhoneNumber)
...
function getMessages(identifier)
	local result = MySQL.Sync.fetchAll("SELECT phone_messages.* FROM phone_messages LEFT JOIN godz_user_identities ON godz_user_identities.user_id = @identifier WHERE phone_messages.receiver = godz_user_identities.phone",{ ['@identifier'] = identifier })
	return result
end
