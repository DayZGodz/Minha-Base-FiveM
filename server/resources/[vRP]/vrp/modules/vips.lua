...
-- [Prepares / DATABASE] --
vRP.prepare("vRP/get_user_vip","SELECT * FROM godz_vips WHERE user_id = @user_id")
...
vRP.prepare("vRP/vip_maxvehs","SELECT COUNT(vehicle) as qtd FROM godz_user_vehicles WHERE user_id = @user_id")
vRP.prepare("vRP/vip_garagem","SELECT * FROM godz_users WHERE id = @user_id")
vRP.prepare("vRP/update_garagem","UPDATE godz_users SET garagem = @garagem WHERE id = @id")
