vRP.prepare("vRP/create_business","INSERT IGNORE INTO godz_business(user_id,capital,laundered,reset_timestamp) VALUES(@user_id,@capital,0,@time)")
vRP.prepare("vRP/get_business","SELECT capital,laundered,reset_timestamp FROM godz_business WHERE user_id = @user_id")
vRP.prepare("vRP/add_capital","UPDATE godz_business SET capital = capital + @capital WHERE user_id = @user_id")
vRP.prepare("vRP/add_laundered","UPDATE godz_business SET laundered = laundered + @laundered WHERE user_id = @user_id")
vRP.prepare("vRP/reset_transfer","UPDATE godz_business SET laundered = 0, reset_timestamp = @time WHERE user_id = @user_id")
