vRP.prepare("vRP/money_init_user","INSERT IGNORE INTO godz_user_moneys(user_id,wallet,bank,coin) VALUES(@user_id,@wallet,@bank,@coin)")
vRP.prepare("vRP/get_money","SELECT wallet,bank,coin FROM godz_user_moneys WHERE user_id = @user_id")
vRP.prepare("vRP/set_money","UPDATE godz_user_moneys SET wallet = @wallet, bank = @bank, coin = @coin WHERE user_id = @user_id")
